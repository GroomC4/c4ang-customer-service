...
/**
 * 관리자 인증 애플리케이션 서비스
 * 관리자의 로그인, 로그아웃 유스케이스를 담당합니다.
 */
@Service
class ManagerAuthenticationService(
    ...
    fun login(command: LoginCommand): LoginResult {
        // 1. 관리자 사용자 조회
        val user =
            loadUserPort.loadByEmailAndRole(command.email, UserRole.MANAGER)
    ...
    fun logout(command: LogoutCommand) {
        ...
        // 2. 역할 확인 (도메인 정책 검증)
        userPolicy.checkUserHasRole(user, UserRole.MANAGER, "ManagerLogout")
