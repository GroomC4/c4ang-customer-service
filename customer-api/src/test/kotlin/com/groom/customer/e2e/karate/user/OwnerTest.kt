...
@E2EScenarioTest
@DisplayName("판매자 인증 플로우 전체 시나리오 테스트")
...
class OwnerTest {
    ...
    fun `owner signup to logout`(): Karate {
        // Spring Boot 서버의 실제 포트를 Karate에 전달
        System.setProperty("karate.port", port.toString())
        // classpath:karate 경로의 owner-authentication.feature 파일 실행
        return Karate.run("classpath:karate/owner-authentication.feature")
    }
}
